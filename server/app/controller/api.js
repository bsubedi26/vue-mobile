...
module.exports = app => {
  class ApiController extends app.Controller {
    ...
    * getAll () {
      ...
      // more than one character will match
      if (q && q.length >= 2) {
      ...
      // pass the groupId select the group under the api,
      // if not, then get the visible group, and then read the api
      if (groupId) {
    ...
    * modifyApi () {
      ...
      delete body._id
      delete body.manager
      // using the lean () method will cause the default value
      // of the schema to be set to prevent clearing empty objects
      const resources = (yield this.service.api.update(apiId, body)).toObject({ minimize: false })
      if (!resources) {
        this.error({
          code: '500',
          msg: 'System error, save failed'
        })
      }
      yield this.notifyApiChange(resources, lastModifiedTime)
      this.service.group.updateTime(groupId)
      // save history and return all records
    ...
    * notifyApiChange (api, lastModifiedTime) {
      ...
      // if the modifier is also in the list of concerns, do not push yourself
      if (selfIdx >= 0) {
    ...
    * delete () {
      ...
      if (!rs) {
        this.error({
          code: 403,
          msg: 'You do not have the right to delete.'
        })
      }
